...
usage() {
 ...
 Submit a job to remove output: ONE leg of an experiment
 
 Options are:
    -o model    : an EC-Earth3 component for which output should be deleted
    ...
    -d depend   : add job dependency (the all point of this)
...
if (( $chck ))
then
    ls -l $OUT/log/odel_$1_$2.out
    grep "\*II\*" $OUT/log/odel_$1_$2.out
    echo
    exit
fi
...
if (( $page ))
then
    echo "Checking $OUT/log/odel_$1_$2.out"
    ${PAGER:-less} $OUT/log/odel_$1_$2.out
    exit
fi

# -- submit script
tgt_script=$OUT/odel_$1_$2

sed "s/<EXPID>/$1/" < delete-ece3.sh.tmpl > $tgt_script
